...
type MockTerminalInstance = {
  ...
  /** Simulates the user typing into the live TUI. */
...
async function activateWorkItemDirectory(path = "/work/app", extra: {
  noteId?: string;
  title?: string;
  sessions?: string[];
  projects?: string[];
  ...
  await act(async () => {
    window.dispatchEvent(new CustomEvent("agent-resume:workbench-work-item", {
      detail: {
        ...
        title: extra.title ?? "Test work item",
        status: "next",
...
describe("WorkbenchPanel", () => {
  ...
  it("activates the assigned project when resuming a moved session", async () => {
    ...
    host.id = "react-workbench";
    document.body.append(host);
    // Session was moved: assigned project /work/docs, native cwd /work/app.
    const workbenchOpenSession = vi.fn(async () => ({
      mode: "embedded",
      cwd: "/work/app",
      command: "codex resume --cd /work/app session-1"
    }));
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
        "desktop.notes.filterProjects": "Filter projects", "desktop.notes.projectFilter": "Project filter", "desktop.common.search": "Search", "desktop.common.all": "All", "desktop.common.active": "Active", "desktop.common.pinned": "Pinned", "desktop.common.close": "Close", "desktop.common.refresh": "Refresh", "desktop.workbench.allSessions": "All sessions", "desktop.workbench.noSessionsInProject": "No sessions", "desktop.workbench.noProjects": "No projects", "desktop.workbench.sidePanelExplorer": "Explorer", "desktop.workbench.sidePanelGit": "Git", "desktop.workbench.newTerminal": "New terminal", "desktop.workbench.newSession": "New session", "desktop.workbench.selectSessionHint": "Select a session", "desktop.workbench.selectProjectHint": "Select a project", "desktop.workbench.externalTerminalHint": "Opened externally", "desktop.workbench.terminalLabel": "Terminal {0}", "desktop.workbench.resumeCommandMissing": "Could not build a resume command", "desktop.workbench.pathMissingHint": "Local folder not found on this machine"
      } }),
      onLocaleChanged: () => () => undefined,
      onWorkbenchCmdT: () => () => undefined,
      onWorkbenchCmdW: () => () => undefined,
      ...
      getSettings: async () => ({ workbench: { defaultNewSessionProvider: "codex" } }),
      listSessions: async () => [
        { provider: "codex" as const, id: "session-1", title: "Moved work", projectPath: "/work/docs", nativeProjectPath: "/work/app", projectOverridden: true, updatedAt: 2 },
        { provider: "claude" as const, id: "session-2", title: "Docs work", projectPath: "/work/docs", updatedAt: 1 }
      ],
      listProjects: async () => [
    ...
    render(<I18nProvider><WorkbenchPanel /></I18nProvider>);
    await act(async () => window.dispatchEvent(new CustomEvent("agent-resume:tab-change", { detail: "workbench" })));
    await activateWorkItemDirectory("/work/docs");
  ...
  it("keeps transcript markdown untouched while the user types and scrolls in the TUI", async () => {
    const host = document.createElement("div");
    host.id = "react-workbench";
    document.body.append(host);
    const terminalInput = vi.fn(async () => ({ ok: true }));
    ...
    await screen.findByRole("button", { name: /Add a transcript pane/ });
    ...
    // Attributes are excluded on purpose: the streaming caret is a container
    ...
    for (let index = 0; index < 6; index += 1) {
      await act(async () => { fireEvent.change(composer as HTMLTextAreaElement, { target: { value: `typing ${index}` } }); });
    }
    // Typing straight into the live TUI: the same path the real terminal uses
    // (PTY input plus the session activity signal).
    for (let index = 0; index < 6; index += 1) {
      await act(async () => { xtermMocks.instances[0]?.emitData(`x${index}`); });
    }
    ...
    for (let index = 0; index < 6; index += 1) {
      await act(async () => {
        terminalHost.dispatchEvent(new WheelEvent("wheel", { deltaY: index % 2 === 0 ? -40 : 40, bubbles: true }));
      });
    }
    ...
    expect(document.querySelector(".wb-transcript-md p")).toBe(paragraph);
  });
  ...
  it("hides session scrollbars from launch and shows only the TUI waterdrop in the alternate buffer", async () => {
    ...
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
        "desktop.notes.filterProjects": "Filter projects", "desktop.notes.projectFilter": "Project filter", "desktop.common.search": "Search", "desktop.common.all": "All", "desktop.common.active": "Active", "desktop.common.pinned": "Pinned", "desktop.common.refresh": "Refresh", "desktop.workbench.allSessions": "All sessions", "desktop.workbench.noSessionsInProject": "No sessions", "desktop.workbench.noProjects": "No projects", "desktop.workbench.sidePanelExplorer": "Explorer", "desktop.workbench.sidePanelGit": "Git", "desktop.workbench.newTerminal": "New terminal", "desktop.workbench.newSession": "New session", "desktop.workbench.selectSessionHint": "Select a session", "desktop.workbench.selectProjectHint": "Select a project", "desktop.workbench.terminalLabel": "Terminal {0}", "desktop.workbench.closeTerminal": "Close terminal", "desktop.workbench.terminalScrollTop": "Scroll to terminal top", "desktop.workbench.terminalScrollBottom": "Scroll to terminal bottom", "desktop.workbench.terminalTuiScrollControl": "TUI scroll control"
      } }),
      onLocaleChanged: () => () => undefined,
      onWorkbenchCmdT: () => () => undefined,
      onWorkbenchCmdW: () => () => undefined,
    ...
    expect(terminal.wheelDeltas.slice(-400)).toEqual(Array(400).fill(-1));
    fireEvent.click(screen.getByRole("button", { name: "Scroll to terminal bottom" }));
    expect(terminal.wheelDeltas.slice(-400)).toEqual(Array(400).fill(1));
    fireEvent.keyDown(waterdrop, { key: "Home" });
    expect(terminal.wheelDeltas.slice(-400)).toEqual(Array(400).fill(-1));
    fireEvent.keyDown(waterdrop, { key: "End" });
    expect(terminal.wheelDeltas.slice(-400)).toEqual(Array(400).fill(1));
  ...
  it("shows clean pending hint in new session, triggers sync on submit, then binds session when catalog syncs", async () => {
    ...
    document.body.append(host);
    let onSessionsSynced: ((result: { syncedAt: number }) => void) | undefined;
    let catalogSessions = [{ provider: "pi", id: "existing", title: "Existing", projectPath: "/work/app", updatedAt: 1 }];
    const syncSessions = vi.fn(async () => ({ sessionCount: 1, syncedAt: Date.now(), warnings: [] }));
    const terminalInput = vi.fn(async () => ({ ok: true }));
    const previewSession = vi.fn(async ({ id }: { id: string }) => ({
      session: { provider: "pi", id, title: "Bound Pi Session", projectPath: "/work/app", updatedAt: Date.now() },
      preview: {
        title: "Bound Pi Session",
        messages: [
          { role: "user", text: "Hello Pi" },
          { role: "assistant", text: "Hello from Grok!" }
        ]
      }
    }));
    window.agentResume = {
      ...
      listSessions: async () => catalogSessions,
      querySessionsPage: async () => ({ sessions: catalogSessions, total: catalogSessions.length }),
      workbenchNewSession: async () => ({ mode: "xterm", command: "pi", cwd: "/work/app" }),
      syncSessions,
      previewSession,
      terminalSpawn: async () => ({ id: 1 }),
      terminalInput,
    ...
    catalogSessions = [
      ...catalogSessions,
      { provider: "pi", id: "pi-session-1", title: "Bound Pi Session", projectPath: "/work/app", updatedAt: Date.now() }
    ];
    await act(async () => onSessionsSynced?.({ syncedAt: Date.now() }));
  ...
  it("shows every dirty repo tree without selecting a repository first", async () => {
    ...
    render(<I18nProvider><WorkbenchPanel /></I18nProvider>);
    await act(async () => window.dispatchEvent(new CustomEvent("agent-resume:tab-change", { detail: "workbench" })));
    await activateWorkItemDirectory("/work/monorepo");
    fireEvent.click(screen.getAllByRole("button", { name: "Git" })[0]!);
  ...
  function gitRepositorySelectionStatus(unstaged: Array<{
    path: string;
    repoPath: string;
    repoRoot: string;
    status: string;
  ...
  it("keeps a manually selected repo selector when another repository becomes dirty", async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const appFile = {
      path: "packages/app/src/app.ts",
      repoPath: "src/app.ts",
  ...
  it("merges a commit from the git graph branch node context menu", async () => {
    const host = document.createElement("div");
    host.id = "react-workbench";
    document.body.append(host);
    const commit = {
    ...
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
        "desktop.notes.filterProjects": "Filter projects", "desktop.notes.projectFilter": "Project filter", "desktop.common.search": "Search", "desktop.common.all": "All", "desktop.common.active": "Active", "desktop.common.pinned": "Pinned", "desktop.common.close": "Close", "desktop.common.refresh": "Refresh", "desktop.workbench.allSessions": "All sessions", "desktop.workbench.noSessionsInProject": "No sessions", "desktop.workbench.noProjects": "No projects", "desktop.workbench.sidePanelExplorer": "Explorer", "desktop.workbench.sidePanelGit": "Git", "desktop.workbench.sidePanelNoChanges": "No changes", "desktop.workbench.sidePanelNoRoot": "Select a project", "desktop.workbench.newTerminal": "New terminal", "desktop.workbench.newSession": "New session", "desktop.workbench.selectSessionHint": "Select a session", "desktop.workbench.selectProjectHint": "Select a project", "desktop.workbench.externalTerminalHint": "Opened externally", "desktop.workbench.terminalLabel": "Terminal {0}", "desktop.workbench.gitLog": "Git log", "desktop.workbench.gitLogTitle": "Git log", "desktop.workbench.gitLogBackToChanges": "Back to changes", "desktop.workbench.gitLogUntitled": "(untitled)", "desktop.workbench.gitLogLoadFailed": "Could not load commit history: {0}", "desktop.workbench.gitStatusRefreshFailed": "Could not refresh Git status: {0}", "desktop.workbench.gitCopyCommitHash": "Copy commit hash", "desktop.workbench.gitCopyBranchName": "Copy branch name", "desktop.workbench.gitMerge": "Merge into current branch", "desktop.workbench.gitMergeConfirm": "Merge commit {0} into the current branch?", "desktop.workbench.gitMergeSucceeded": "Merge completed.", "desktop.workbench.gitMergeFailed": "Could not merge commit: {0}", "desktop.workbench.gitRevert": "Revert"
      } }),
      onLocaleChanged: () => () => undefined,
      onWorkbenchCmdT: () => () => undefined,
      onWorkbenchCmdW: () => () => undefined,
  ...
  it("opens a Git change from its context menu in Workbench or the default app", async () => {
    ...
    render(<I18nProvider><WorkbenchPanel /></I18nProvider>);
    await act(async () => window.dispatchEvent(new CustomEvent("agent-resume:tab-change", { detail: "workbench" })));
    await activateWorkItemDirectory("/work/app/apps/desktop");
    fireEvent.click(screen.getAllByRole("button", { name: "Git" })[0]!);
  ...
  it("switches projects from the command palette and closes Quick Access", async () => {
    ...
    input = document.querySelector(".quick-access-input") as HTMLInputElement;
  ...
  it("expands newly appeared Git directories by default while keeping collapsed ones collapsed", async () => {
    const host = document.createElement("div");
    host.id = "react-workbench";
    document.body.append(host);
    const gitFile = {
    ...
    const terminalGitStatus = vi.fn(async () => ({
      ...
      root: "/work/app",
      staged: [],
      unstaged: hasNewDir ? [gitFile, newFile] : [gitFile],
      nestedRepos: [],
      tracking: []
    }));
    window.agentResume = {
    ...
    expect(directoryToggle.getAttribute("aria-expanded")).toBe("true");
    // Collapse the user's directory; the new directory must still default to expanded.
    fireEvent.click(directoryToggle);
    await waitFor(() => expect(directoryToggle.getAttribute("aria-expanded")).toBe("false"));
    ...
    await waitFor(() => expect(terminalGitStatus.mock.calls.length).toBeGreaterThan(statusCalls));
    await waitFor(() => {
      const srcButton = document.querySelector<HTMLButtonElement>('.wb-file-tree-label[title="src"]')?.closest("button");
      const libButton = document.querySelector<HTMLButtonElement>('.wb-file-tree-label[title="lib"]')?.closest("button");
      if (!srcButton || !libButton) throw new Error("Git directory toggles missing");
      expect(srcButton.getAttribute("aria-expanded")).toBe("false");
      expect(libButton.getAttribute("aria-expanded")).toBe("true");
    });
    expect(screen.queryByTitle("src/app.ts")).toBeNull();
    expect(screen.getByTitle("lib/util.ts")).toBeTruthy();
  });

  it("stages two files clicked in quick succession without dropping the second click", async () => {
    const host = document.createElement("div");
    host.id = "react-workbench";
    document.body.append(host);
    const files = [
      { path: "a.ts", repoPath: "a.ts", repoRoot: "/work/app", status: "M", staged: false, unstaged: true },
      { path: "b.ts", repoPath: "b.ts", repoRoot: "/work/app", status: "M", staged: false, unstaged: true }
    ];
    const stagedPaths = new Set<string>();
    let releaseFirstStage: () => void = () => {};
    const firstStageGate = new Promise<void>((resolve) => { releaseFirstStage = resolve; });
    let firstStage = true;
    const terminalGitStage = vi.fn(async (args: { repoRoot: string; paths: string[] }) => {
      if (firstStage) {
        firstStage = false;
        await firstStageGate;
      }
      for (const path of args.paths) stagedPaths.add(path);
      return { ok: true };
    });
    const terminalGitStatus = vi.fn(async () => ({
      isRepo: true,
      root: "/work/app",
      staged: files.filter((file) => stagedPaths.has(file.path)).map((file) => ({ ...file, staged: true, unstaged: false })),
      unstaged: files.filter((file) => !stagedPaths.has(file.path)),
      nestedRepos: [],
      tracking: []
    }));
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
        ...
        "desktop.workbench.gitStageFailed": "Stage failed",
        "desktop.workbench.gitUnstageFailed": "Unstage failed"
      } }),
      onLocaleChanged: () => () => undefined,
      onWorkbenchCmdT: () => () => undefined,
      onWorkbenchCmdW: () => () => undefined,
    ...
    // First stage call is in flight when the second checkbox is clicked.
    fireEvent.click(screen.getByRole("checkbox", { name: "a.ts" }));
    fireEvent.click(screen.getByRole("checkbox", { name: "b.ts" }));
    await act(async () => releaseFirstStage());

    await waitFor(() => expect(terminalGitStage).toHaveBeenCalledTimes(2));
    expect(terminalGitStage).toHaveBeenNthCalledWith(1, { repoRoot: "/work/app", paths: ["a.ts"] });
    expect(terminalGitStage).toHaveBeenNthCalledWith(2, { repoRoot: "/work/app", paths: ["b.ts"] });
    // Both files end up in the Staged section after the optimistic moves converge
    // with the authoritative refresh.
    await waitFor(() => {
      expect(screen.getByRole("checkbox", { name: "a.ts" }).getAttribute("aria-checked")).toBe("true");
      expect(screen.getByRole("checkbox", { name: "b.ts" }).getAttribute("aria-checked")).toBe("true");
    });
  });

  it("discards a stale git status from a previous project after switching projects", async () => {
    const host = document.createElement("div");
    host.id = "react-workbench";
    document.body.append(host);
    const appFile = {
      path: "app-src/old.ts",
      repoPath: "app-src/old.ts",
      repoRoot: "/work/app",
      status: "M",
      staged: false,
      unstaged: true
    ...
    const docsFile = {
      path: "docs-mod/new.ts",
      repoPath: "docs-mod/new.ts",
      repoRoot: "/work/docs",
      status: "M",
      staged: false,
      unstaged: true
    ...
    const gate = (project: string) => new Promise<void>((resolve) => {
      const list = pending.get(project) || [];
      list.push(resolve);
      pending.set(project, list);
    });
    const terminalGitStatus = vi.fn(async (args: { cwd: string }) => {
      await gate(args.cwd);
      return args.cwd === "/work/app"
        ? { isRepo: true, root: "/work/app", staged: [], unstaged: [appFile], nestedRepos: [], tracking: [] }
        : { isRepo: true, root: "/work/docs", staged: [], unstaged: [docsFile], nestedRepos: [], tracking: [] };
    });
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
        "desktop.notes.filterProjects": "Filter projects",
      ...
      listSessions: async () => [
        { provider: "codex", id: "session-1", title: "Fix renderer", projectPath: "/work/app", updatedAt: 1 },
        { provider: "codex", id: "session-2", title: "Docs work", projectPath: "/work/docs", updatedAt: 1 }
      ],
      terminalGitStatus,
      terminalGitFetch: async () => ({ ok: true })
    } as unknown as typeof window.agentResume;
    ...
    await waitFor(() => expect(screen.queryByTitle("app-src/old.ts")).toBeNull());
    // The stale result was discarded: no git tree rows (and no legacy file rows) are rendered.
    expect(document.querySelector(".wb-file-tree-row")).toBeNull();
    expect(document.querySelector(".wb-git-file")).toBeNull();
    expect(document.querySelector<HTMLElement>(".react-git-panel")?.textContent).toContain("Git unavailable");

    // The next project's first status refresh defaults to a fully expanded tree.
    fireEvent.click(document.querySelector<HTMLButtonElement>('.wb-git-actions button[aria-label="Refresh"]')!);
    await waitFor(() => expect(terminalGitStatus.mock.calls.some(([args]) => args.cwd === "/work/docs")).toBe(true));
    await act(async () => { pending.get("/work/docs")?.forEach((resolveGate) => resolveGate()); });
    const docsToggle = await waitFor(() => {
      const button = document.querySelector<HTMLButtonElement>('.wb-file-tree-label[title="docs-mod"]')?.closest("button");
      if (!button) throw new Error("Docs Git directory toggle missing");
      return button;
    });
    expect(docsToggle.getAttribute("aria-expanded")).toBe("true");
    expect(screen.getByTitle("docs-mod/new.ts")).toBeTruthy();
    expect(screen.queryByTitle("app-src/old.ts")).toBeNull();
  });
  ...
  it("organizes the Workbench by work items", async () => {
    ...
    // Finder is the entry point to attach a local directory to a project-less work item.
    const addProject = document.querySelector(".wb-work-item-add-project") as HTMLButtonElement;
    ...
    await waitFor(() => expect(document.querySelector(".wb-work-item")).not.toBeNull());
    expect(screen.queryByRole("tab", { name: "Repository" })).toBeNull();
    expect(document.querySelector(".wb-work-item-section")).not.toBeNull();
  });
  ...
  it("sorts work items by live session rank desc then updatedAtMs desc", async () => {
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
        "desktop.common.search": "Search", "desktop.common.refresh": "Refresh", "desktop.common.all": "All",
        "desktop.workbench.allSessions": "All sessions",
      ...
      agentStatusGetSnapshot: async () => ({
        byPaneId: {},
        bySessionKey: {
          "codex:s-live": { state: "blocked", source: "native", authority: "native", agent: "codex" }
        },
        seq: 1
      }),
      ...
      notesListWorkItems: async () => [
        { noteId: "wi-old", title: "Old item", gtdStatus: "next", updatedAtMs: 100, work: { sessions: [] } },
        { noteId: "wi-awaiting", title: "Awaiting item", gtdStatus: "next", updatedAtMs: 50, work: { sessions: ["codex:s-live"] } },
        { noteId: "wi-new", title: "New item", gtdStatus: "next", updatedAtMs: 200, work: { sessions: [] } }
      ],
      terminalSpawn: async () => ({ id: 1 }),
      terminalGitInfo: async () => ({ mode: "none", isRepo: false, branch: null, repoRoot: null, nestedRepos: [] }),
      terminalDestroy: async () => ({ ok: true }),
    ...
    let labels = Array.from(document.querySelectorAll(".wb-work-item-row .wb-folder-row-label")).map((el) => el.textContent);
    expect(labels).toEqual(["New item", "Old item", "Awaiting item"]);

    await act(async () => {
      window.dispatchEvent(new CustomEvent("agent-resume:workbench-open-session", {
        detail: { provider: "codex", id: "s-live", projectPath: "/work/app" }
      }));
    });

    await waitFor(() => {
      labels = Array.from(document.querySelectorAll(".wb-work-item-row .wb-folder-row-label")).map((el) => el.textContent);
