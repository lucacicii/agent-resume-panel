...
describe("WorkbenchPanel", () => {
  ...
  it("opens a floating note from an active CLI session tab and creates the session note", async () => {
    ...
    const openFloatingNoteWindow = vi.fn(async () => ({ ok: true }));
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages: {
    ...
    await waitFor(() => expect(openFloatingNoteWindow).toHaveBeenCalledWith({
      provider: "codex",
      sessionId: "session-1",
      projectPath: "/work/app",
      projectName: "app",
