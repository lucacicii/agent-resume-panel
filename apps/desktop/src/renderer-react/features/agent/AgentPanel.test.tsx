...
describe("AgentPanel", () => {
  ...
  it("loads a thread and sends a streamed Agent request", async () => {
    ...
    expect(document.querySelector(".chat-compose-context")?.getAttribute("title")).toBe("All projects");
    expect((document.querySelector(".chat-compose-context select") as HTMLSelectElement | null)?.value).toBe("");
  ...
  it("scopes the ask context to the project selected in the compose bar", async () => {
    const host = document.createElement("div"); host.id = "react-agent"; document.body.append(host);
    const askAgent = vi.fn(async () => ({ answer: "Project-scoped", citations: [], fallback: false, digests: [] }));
    const listProjects = vi.fn(async () => [{
      projectId: "p-1", portableKey: "~/proj-a", alias: "Proj A", hidden: false, pinned: false,
      lastSeenAtMs: null, updatedAtMs: 1, localPath: "/Users/me/proj-a", pathMissing: false, sessionCount: 0
    }]);
    window.agentResume = {
      getI18nBundle: async () => ({ locale: "en", messages }), onLocaleChanged: () => () => undefined,
      listAgentThreads: async () => [thread], listProjects,
      listAgentChat: async () => ({ messages: [], hasMore: false }), listOlderAgentChat: async () => ({ messages: [], hasMore: false }),
      createAgentThread: async () => thread, deleteAgentThread: async () => ({ ok: true }), renameAgentThread: async () => ({ ok: true }),
      askAgent, cancelAskAgent: async () => ({ ok: true }), onAskStream: () => () => undefined, clearAgentChat: async () => ({ ok: true })
    } as unknown as typeof window.agentResume;
    render(<I18nProvider><AgentPanel /></I18nProvider>);
    await act(async () => window.dispatchEvent(new CustomEvent("agent-resume:tab-change", { detail: "agent" })));
    const select = (await screen.findByLabelText("Conversation project")) as HTMLSelectElement;
    expect(Array.from(select.options).map((option) => option.textContent)).toContain("Proj A");
    fireEvent.change(select, { target: { value: "/Users/me/proj-a" } });
    fireEvent.change(screen.getByPlaceholderText("Type a message"), { target: { value: "Summarize" } });
    fireEvent.click(screen.getByRole("button", { name: "Send" }));
    await waitFor(() => expect(askAgent).toHaveBeenCalledWith(expect.objectContaining({
      query: "Summarize",
      threadId: "thread-1",
