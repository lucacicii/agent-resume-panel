...
export function NotesPanel(): ReactPortal | null {
  ...
  const save = useCallback(async () => {
    ...
    if (!note || note.systemManaged) return;
    try {
  ...
  const open = useCallback(async (note: Note) => {
    ...
    try {
      ...
      setView(result.record.systemManaged ? "view" : "edit");
      setFindOpen(false);
      setFindQuery("");
    } catch (error) { setStatus({ text: error instanceof Error ? error.message : String(error), kind: "error" }); }
  }, [save]);
    ...
    <section className="panel active notes-panel react-notes-panel" hidden={!active}>
      <div className="notes-layout">
        ...
        <main className="notes-detail">
          {selected ? <div className="notes-editor-shell"><div className="notes-detail-head">{editingTitle && !selected.systemManaged ? <form onSubmit={(event) => { event.preventDefault(); void rename(); }}><input className="notes-detail-title-input" value={title} onChange={(event) => setTitle(event.target.value)} autoFocus /><button type="submit" className="notes-icon-btn" aria-label={t("desktop.common.confirm")}><Save size={15} /></button></form> : <h1 className="notes-detail-title" onDoubleClick={() => { if (!selected.systemManaged) setEditingTitle(true); }}>{title}</h1>}{selected.systemManaged ? <span className="muted">{t("desktop.notes.systemManaged")}</span> : null}<div className="notes-segmented" role="tablist">{!selected.systemManaged ? <button type="button" role="tab" className={view === "edit" ? "active" : ""} aria-label={t("desktop.common.edit")} onClick={() => setView("edit")}><Pencil size={16} /></button> : null}<button type="button" role="tab" className={view === "view" ? "active" : ""} aria-label={t("desktop.common.view")} onClick={() => { if (!selected.systemManaged) void save(); setView("view"); }}><Eye size={16} /></button><button type="button" className="notes-icon-btn" aria-label={t("desktop.notes.findInNote")} onClick={() => setFindOpen(true)}><Search size={15} /></button><button type="button" className="notes-icon-btn" aria-label={t("desktop.notes.copyPath")} onClick={() => void desktopApi().notesCopyPath({ noteId: selected.noteId })}><Clipboard size={15} /></button><button type="button" className="notes-icon-btn" aria-label={t("desktop.common.revealInFinder")} onClick={() => void desktopApi().notesReveal({ noteId: selected.noteId })}><FolderOpen size={15} /></button>{!selected.systemManaged ? <button type="button" className="notes-icon-btn" aria-label={t("desktop.notes.deleteNote")} onClick={() => void remove()}><Trash2 size={15} /></button> : null}</div></div>
            ...
            {view === "edit" && !selected.systemManaged ? <CodeEditor ref={editorRef} className="notes-editor-host" value={content} language="markdown" ariaLabel={t("desktop.notes.editorPlaceholder")} onChange={editContent} onBlur={() => void save()} shouldHandlePaste={() => desktopApi().notesClipboardHasImage()} onPasteImage={pasteImage} slashCommands={gtdSlashCommands} /> : <div className="notes-preview markdown-body" onClick={(event) => { if (event.target instanceof HTMLImageElement) setImagePreview(event.target.src); }} dangerouslySetInnerHTML={{ __html: renderMarkdown(content) }} />}
      ...
      {contextMenu ? <div className="notes-context-menu" role="menu" style={{ left: Math.max(8, Math.min(contextMenu.x, window.innerWidth - 220)), top: Math.max(8, Math.min(contextMenu.y, window.innerHeight - 260)) }} onContextMenu={(event) => event.preventDefault()}>
        {contextMenu.kind === "project" ? <>
          ...
          <div className="context-menu-separator" role="separator" />
          {!contextMenu.note.systemManaged ? <button type="button" role="menuitem" onClick={() => { beginTarget("move", contextMenu.note); setContextMenu(null); }}><span>{t("desktop.notes.changeOwner")}</span><ChevronRight className="context-menu-chevron" size={14} aria-hidden="true" /></button> : null}
          {!contextMenu.note.systemManaged ? <button type="button" role="menuitem" onClick={() => { setRenameDialog({ kind: "note", note: contextMenu.note, title: titleFor(contextMenu.note) }); setContextMenu(null); }}>{t("desktop.common.rename")}</button> : null}
          ...
          {!contextMenu.note.systemManaged ? <><div className="context-menu-separator" role="separator" /><button type="button" role="menuitem" className="context-menu-item-danger" onClick={() => { void remove(contextMenu.note); setContextMenu(null); }}>{t("desktop.notes.deleteNote")}</button></> : null}
        </>}
      </div> : null}
