/**
 * Screen-content fingerprinting.
 *
 * Last-resort detection for agents that expose no hook and emit no status
 * sequence — agents whose approval dialogs and choice menus are only visible as
 * text. Pure string analysis: give it text, get a verdict. It never reads a
 * terminal, never holds state, and never touches the UI.
 *
 * Keep every pattern *multi-signal*. A single loose keyword produces false
 * "blocked" verdicts, and a wrong "waiting for you" costs more than a missed cue.
 *
 * Stage 4 replaces this module with declarative per-agent manifests.
 */
