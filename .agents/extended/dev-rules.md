...
- Desktop **Chat** needs the external Thunder daemon (`thunder-agent-daemon`, a separate Rust repo). Discovery rules, env overrides (`THUNDER_PATH`, `THUNDER_DAEMON_BIN`, `THUNDER_DEBUG=1`), and the testing checklist are in [`apps/desktop/DEVELOPMENT.md`](../../apps/desktop/DEVELOPMENT.md) § Thunder daemon; the resolver lives in `apps/desktop/src/main/thunder/daemonResolver.ts` with unit tests next to it. Never hardcode a developer-specific absolute path there.
- Release packaging embeds that daemon (`apps/desktop/scripts/thunder-sidecar.mjs` → `Contents/Resources/thunder/bin/thunder-daemon`, one binary per arch). Run `pnpm --filter @agent-resume/desktop run test:thunder-sidecar` after touching it; `AGENT_RESUME_REQUIRE_THUNDER=1 pnpm run pack:desktop` must succeed before a release. `dev:mac` intentionally packs without the daemon.
