...
| renderer runtime | `apps/desktop/src/renderer-react/main.tsx`, `apps/desktop/src/renderer/index.html` | React runtime, one entry per window mode: `main` (the board nav sidebar plus the GTD or Notes view), `task` (one workbench), `standalone-note`, `browser`. |
| board nav sidebar | `apps/desktop/src/renderer-react/components/AppSidebar.tsx`, `components/useGlideHighlight.ts` | Full-height GTD/Notes sidebar (collapsible icon rail, gliding hover highlight). The macOS traffic lights sit over its top strip, so the header starts beside the sidebar; `data-board-view` on `<html>` decides which host is visible. The collapse state is owned by `MainRendererRuntime` and toggled from the header's leading button (`AppChrome`). |
| board view switching | `apps/desktop/src/renderer-react/main.tsx` (`MainRendererRuntime`), `components/AppSidebar.tsx` | One board window shows GTD (`features/gtd/GtdView.tsx`) or Notes (`features/notes/NotesView.tsx`). The view comes from View ▸ Show GTD Board / Show Notes (⌘1/⌘2, menu IPC `nav:show`) or the `agent-resume:board-view` event from the quick access palette. |
| primary navigation shell | `apps/desktop/src/renderer-react/components/AppChrome.tsx` | Board window header beside the sidebar (bell, account/settings menu, floating-note dots). Workbenches are separate windows, so the board opens and focuses those instead of switching views inside itself. |
