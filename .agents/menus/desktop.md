...
| renderer runtime | `apps/desktop/src/renderer-react/main.tsx`, `apps/desktop/src/renderer/index.html` | React runtime. Host divs (`react-chrome`, `react-report`, `react-workbench`, `react-notes`, `react-im`, `react-settings`) anchor each panel's portal. |
| primary navigation shell | `apps/desktop/src/renderer-react/components/AppChrome.tsx` | Rail tabs are `report` / `workbench` / `notes`; **Workbench is the default landing tab**. Other surfaces switch tabs through the `agent-resume:tab-request` / `agent-resume:tab-change` events, not by direct state access. |
| Archive / Report panel | `apps/desktop/src/renderer-react/features/report/ReportPanel.tsx` | Work-item-first history: left work item list (plus a pinned unassigned entry), middle session list with whole-catalog search, right detail pane (GTD capsule, projects, next action, decision, note/IM entries, session timeline, report pointers). Session pages load 200 at a time with an explicit **Load earlier** button. |
| work item live rollup | `apps/desktop/src/renderer-react/features/workbench/sessionStatus/workItemRollup.ts` | Shared `rollupDot()` / `rank()` / `needsYou()` / `LIVE_RANK` used by both the Workbench work item list and the Archive left column. Live dots only cover sessions open in this run; `lastExitWaiting` covers sessions that were waiting when the app last quit. |
| work item detail modal | `apps/desktop/src/renderer-react/features/noteDetail/` | `NoteDetailSheet` (formerly `KanbanCardModal`, renamed in P8) is a note/session detail sheet. **It currently has no importer** — see [`desktop-archive-followups.md`](../../.arp/docs/desktop-archive-followups.md) T4. |
