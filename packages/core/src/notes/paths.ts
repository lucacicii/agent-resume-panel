...
import { basenameOrPath, expandHome, normalizeProjectPath } from "../pathUtils";
...
export function ownerRelDir(owner: NoteOwner): string {
  ...
  return sessionNoteRelDir(owner.provider, owner.sessionId);
}
...
export function parseOwnerJson(raw: unknown): NoteOwner | undefined {
  ...
  if (
    ...
    return {
      scope: "session",
      provider: obj.provider as AgentProvider,
