import type { ReportEntry } from "./schema";
...
export function parseDayRange(key: string): CalendarPeriodRange | null {
...
export function parseWeekRange(key: string): CalendarPeriodRange | null {
...
export function parseMonthRange(key: string): CalendarPeriodRange | null {
...
export function rangeForPeriod(type: ReportPeriodType, key: string): CalendarPeriodRange | null {
  if (type === "day") {
    return parseDayRange(key);
  }
  if (type === "week") {
    return parseWeekRange(key);
  }
  return parseMonthRange(key);
}

export function paddedMonthRange(year: number, monthIndex: number): CalendarPeriodRange {
...
export function periodKeyFromEntry(
  entry: Pick<ReportEntry, "id" | "level" | "periodStartMs">
): string {
  ...
  if (level === "daily" && /^\d{4}-\d{2}-\d{2}$/.test(fromId)) {
    return fromId;
  }
  if (level === "weekly" && /^\d{4}-W\d{2}$/i.test(fromId)) {
    return fromId;
  }
  if (level === "monthly" && /^\d{4}-\d{2}$/.test(fromId)) {
    return fromId;
  }
  ...
  if (level === "weekly") {
    return isoWeekLabelFromDate(date);
  }
  if (level === "monthly") {
...
/** Map of `${level}:${periodKey}` → entry for daily/weekly/monthly digests. */
...
export function isFuturePeriod(type: ReportPeriodType, key: string, now = new Date()): boolean {
  ...
  return key > isoWeekLabelFromDate(now);
}
